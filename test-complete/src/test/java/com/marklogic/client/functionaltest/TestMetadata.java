...
public class TestMetadata extends BasicJavaClientREST{
	...
	/* For Git Issue #550
	 *  Write a document with metadata that says a permission gives a specific role the node-update capability. 
	 *  Read the metadata for the document and see that it has that permission with that roles assigned node-update capability
	 *  Use the new role to update the document
	 *  Use the new role to change document metadata. 
	 */
	@Test	
	...
	{	
		...
		DatabaseClient clientW = null, elsclient = null;
		try {
		...
		for(String filename : filenames) {
			writeDocumentUsingInputStreamHandle(clientW, filename, "/nodeUpdate/", "JSON");
		}
		
		String docId = "/nodeUpdate/json-original.json";
		JSONDocumentManager docMgr = clientW.newJSONDocumentManager();
		...
		elsclient = getDatabaseClientWithDigest("elsuser", "x");
