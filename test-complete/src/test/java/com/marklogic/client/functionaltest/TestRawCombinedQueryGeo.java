...
public class TestRawCombinedQueryGeo extends BasicJavaClientREST {
	...
	{	
		...
		DatabaseClient clientW = getDatabaseClientWithDigest("rest-writer", "x");
		...
		QueryManager queryMgr = clientW.newQueryManager();
	...
	{	
		...
		// write docs
		for(int i = 1; i <= 9; i++) {
			writeDocumentUsingInputStreamHandle(clientW, "geo-constraint" + i + ".xml", "/geo-constraint/", "XML");
		}
