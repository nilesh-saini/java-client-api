...
public class WriteBatcherJobReportTest extends  DmsdkJavaClientREST{
		        		...
		        		if (dmManager.getJobReport(writeTicket).getSuccessBatchesCount() ==1){
		        			if (dmManager.getJobReport(writeTicket).getFailureEventsCount() ==10){
		        				if (dmManager.getJobReport(writeTicket).getFailureBatchesCount() ==1){
		        					if(Math.abs(dmManager.getJobReport(writeTicket).getReportTimestamp().getTime().getTime()-Calendar.getInstance().getTime().getTime()) < 200){
	...
	public void testJobReportStopJob() throws Exception {
		...
		Assert.assertTrue(dbClient.newServerEval().xquery(query1).eval().next().getNumber().intValue() == dmManager.getJobReport(writeTicket).getSuccessEventsCount());
	...
	public String getSummaryReport(Object[] report) {
		  ...
		  return ("batches: "     + report[0] +
		    ", docs: "           + report[1] +
		    ", failed docs: "    + report[2] +
		    ", failed batches: " + report[3] +
		    ", timestamp: " + report[4]);
		}
}
